...
export default function UsersTable({ users, onFilter, onViewDetails }: { users: User[]; onFilter: () => void; onViewDetails: () => void }) {
      ...
      <tbody>
        {users.map((u) => (
          <tr key={u.id}>
            ...
            <td className="menu-cell"><button className="dots" onClick={() => setMenuId(menuId === u.id ? null : u.id)}>⋮</button>{menuId === u.id && <RowActionMenu onViewDetails={onViewDetails} />}</td>
          </tr>
        ))}
      </tbody>
    </table>
